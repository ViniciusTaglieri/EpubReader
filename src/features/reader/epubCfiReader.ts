...
export function applyReaderSettings(
  ...
  rendition.flow?.(
    settings.flow === 'continuous' ? 'scrolled-doc' : 'paginated',
  )
...
export function sanitizeRenderedEpubDocument(doc: Document) {
  doc.querySelectorAll(UNSAFE_EPUB_SELECTOR).forEach((node) => {
    node.remove()
  })
...
export function pageStatsFromLocation(
  ...
  const locationIndex = Math.max(
    0,
    epub?.locations?.locationFromCfi?.(cfi) ?? 0,
  )
